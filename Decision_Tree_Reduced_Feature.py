...
def main():
    ...
    tree_model = DecisionTree.trainClassifier(training_data,numClasses=2, categoricalFeaturesInfo={},
                                     impurity='gini', maxDepth=04, maxBins=100)
    tt= time() - t0
    tree_model.save(sc,"DTmodel_model_reduced_222red")  #Save the model for future use!
    ...
    #Manually specify the correct datatypes for each Column and import new DF
    fields = [StructField(field_name, FloatType(), True) for field_name in features_new]
    fields[0].dataType = IntegerType()
    fields[-1].dataType = IntegerType()
    customSchema = StructType(fields)
    df.select(df.columns).write.format('com.databricks.spark.csv').options(header='true').save('/data/ganesh/BigData/Bosch/Source/feature_reduction/numeric/feature_reduction_V2/df_739.csv',schema = customSchema)
    sc.stop()
